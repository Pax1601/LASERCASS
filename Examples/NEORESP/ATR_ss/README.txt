...
ATR.dat: NeoRESP main file
ATR.inc: SMARTCAD aeroelastic model
ATRCONM_CONF1.inc: mass configuration
GustSolverParam.inc: settings for gust response

Steps:

1) launch NeoRESP preprocessor: 
   init_dyn_model('ATR.dat')

2) save the database:
   global dyn_model;
   save('ATR.mat', 'dyn_model');
...
one for Qag due to gust, ATR_Ham_M_0.7.mat and ATR_Hag_M_0.7.mat.
...
4) open the script aero_ss.m and look at the parameters used for fitting.
   ...
   while for Hag:
    ...
    Run the script for Ham:
      aero_ss('ATR_Ham_M_0.7.mat','res1.mat')
    ...
    Run the script for Hag:
      aero_ss('ATR_Hag_M_0.7.mat','res2.mat')
    Keep all the Hankel singular values and
    check for the quality of the interpolation by looking into the interpolated terms.
