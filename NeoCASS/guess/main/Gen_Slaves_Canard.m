...
%   <andreadr@kth.se>
%--------------------------------------------------------------------------------------------------------------------------------
function [stick] = Gen_Slaves_Canard(stick, geo)

% Identification number for slave nodes
IDslaveW = 49999;

dihedral = zeros(geo.canard.leny, 1);
for j = 1:length(geo.canard.CAERO1.n)-1
    dihedral(geo.canard.index(j):geo.canard.index(j+1),1) = geo.canard.CAERO1.dihedral(j);
end

Y  = [0; cumsum(stick.canr.Lbeam_thick)];
Yc = [0; cumsum(stick.canr.Lbeam)];
deltaX     = interp1(Y, geo.canard.Zs,  Yc, 'linear', 'extrap');
deltaXX    = interp1(Y, geo.canard.rs,  Yc, 'linear', 'extrap');
deltaZ     = interp1(Y, geo.canard.tbs,  Yc, 'linear', 'extrap');
deltaAlpha = interp1(Y, geo.canard.incidence,  Yc, 'linear', 'extrap');
...
if isequal(stick.model.canl, 1)
    ...
    stick.slaves.ID.canl = [stick.slaves.ID.canr(end)+1:stick.slaves.ID.canr(end)+length(stick.slaves.ID.canr)]';
