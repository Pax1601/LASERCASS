...
function writeRIGIDlinkCELAS_mod(fid, stick, geo, pdcylin)
...
for i = 1 : length(stick.link_thick.Ma_thick)
    N = length(stick.link_thick.RBE2(i).slave);
    for j = 1 : N
        writeCELAS(fid,cont,K,stick.link_thick.Ma(i),stick.link_thick.RBE2(i).slave(j),stick.link_thick.RBE2(i).DOF);
        cont = cont+1;
    end
