...
function [int_forces] =internal_f_twin_htails(forces, state, stick, distances, forces_vtail2, aircraft)
...
%     end
   ...
   %Inversione delle forze aerodinamiche
    ...
    if isequal(stick.model.horr, 1)
        int_forces.Tx(1,:)=forces(1,1)-forces_vtail2.Tx(1);
        int_forces.Ty(1,:)=forces(1,2)-forces_vtail2.Ty(1);
        int_forces.Tz(1,:)=forces(1,3)-forces_vtail2.Tz(1);
        int_forces.Mx(1,:)=forces(1,2)-forces_vtail2.Mx(1);
        int_forces.My(1,:)=forces(1,2)-forces_vtail2.My(1);
        int_forces.Mz(1,:)=forces(1,3)-forces_vtail2.Mz(1);
    else
    ...
    for i=2:length(forces)
        ...
        int_forces.Mx(i,:)=int_forces.Mx(i-1,:)+forces(i,2)*distances.span(end-i+2);
        int_forces.My(i,:)=int_forces.My(i-1,:)+forces(i,2)*distances.x(end-i+2);
