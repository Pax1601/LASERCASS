...
function [int_forces] =internal_f_vtail(forces, state, stick, distances, forces_htail_dx, forces_htail_sx, aircraft)
...
%     end
   ...
   %Inversione delle forze aerodinamiche
    ...
    if isequal(stick.model.horr, 1)
        int_forces.Tx(1,:)=forces(1,1)-forces_htail_dx.Tx(1)-forces_htail_sx.Tx(1);
        int_forces.Ty(1,:)=forces(1,2)-forces_htail_dx.Ty(1)-forces_htail_sx.Ty(1);
        int_forces.Tz(1,:)=forces(1,3)-forces_htail_dx.Tz(1)-forces_htail_sx.Tz(1);
        int_forces.Mx(1,:)=forces(1,2)-forces_htail_dx.Mx(1)-forces_htail_sx.Mx(1);
        int_forces.My(1,:)=forces(1,2)-forces_htail_dx.My(1)-forces_htail_sx.My(1);
        int_forces.Mz(1,:)=forces(1,3)-forces_htail_dx.Mz(1)-forces_htail_sx.Mz(1);
    else
