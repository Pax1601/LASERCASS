...
function [int_forces] =internal_f_twin_htailr(forces, state, stick, distances, forces_vtail, aircraft)
...
%     end
   ...
   %Inversione delle forze aerodinamiche
    ...
    if isequal(stick.model.horr, 1)
        int_forces.Tx(1,:)=forces(1,1)-forces_vtail.Tx(1);
        int_forces.Ty(1,:)=forces(1,2)-forces_vtail.Ty(1);
        int_forces.Tz(1,:)=forces(1,3)-forces_vtail.Tz(1);
        int_forces.Mx(1,:)=forces(1,2)-forces_vtail.Mx(1);
        int_forces.My(1,:)=forces(1,2)-forces_vtail.My(1);
        int_forces.Mz(1,:)=forces(1,3)-forces_vtail.Mz(1);
    else
