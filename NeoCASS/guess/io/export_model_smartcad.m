...
%*******************************************************************************
%
function [file_name_conf, WREG, WREGCG] = export_model_smartcad(stick, geo, aircraft, ref, pdcylin, str,  filename_stick, filename_trim, nconf, niter)
%
fid = 1;
%
...
%    
  ...
  % PBAR / PBARSMX card
  [geo, str, stick] = setup_PROPERTY_BAR_mod(fid, fp, geo, str, stick, aircraft, pdcylin, niter, 1);
  % CAERO1 card
  ...
  % CONM2
  if (niter==1)
    [str,IDSUPORT,stick] = Add_NSM_conc(fid, fp2, fp2, pdcylin, aircraft, geo, stick, str, 0, 0);
  else 
    [str,IDSUPORT,stick] = Add_NSM_conc(fid, fp2, fp2, pdcylin, aircraft, geo, stick, str, 0, 1);
  end
  writeRIGIDlinkRBE2(fp, stick, 0);
  % TRIM card
  if ~isempty(filename_trim)
    writeINCLUDE2file(fp, filename_trim);
  end
  % AEROS card
...
%
  ...
  [geo, str, stick] = setup_PROPERTY_BAR_mod(fid, fp2, geo, str, stick, aircraft, pdcylin, niter, 0);
  if (niter==1)
    [str,IDSUPORT,stick] = Add_NSM_conc(fid, fp2, fp2, pdcylin, aircraft, geo, stick, str, 0, 0);
  else      
    [str,IDSUPORT,stick] = Add_NSM_conc(fid, fp2, fp2, pdcylin, aircraft, geo, stick, str, 0, 1);
