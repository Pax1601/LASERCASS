...
function [cross_section] = plot_geo_section(concept, tbar_S, d, Nxc, E)

% consider section with maximum compressive load
[Nxc,J] = max(loads.fus.Nxc);
tbar_S = str.fus.tbar_S(J);
d = str.fus.d(J);
E = pdcylin.fus.es;
...
% % Detailed cross-section plot
% plot_sect_unflanged(ts, tw, bs, bw)
...
% % Detailed cross-section plot
% plot_sect_stiffened(ts, tf, tw, bs, bf, bw)
...
% % Detailed cross-section plot
% plot_sect_truss(tf, tc, bf, bc, h, theta)

end
%------------------------------------------------------------------------------------------------------------
function [tf, tc, bf, bc, h, theta] = kcon8(tbar_S, Nxc, E)
%--------------------------------------------------------------------------
% Applicable to fuselage structural concept: 8
%   ...
% 
% Output: 6 parameters
% 
...
%--------------------------------------------------------------------------
% Applicable to fuselage structural concept: 9
...
%*****************************************%
% Functions for the geometric description %
%*****************************************%

%------------------------------------------------------------------------------------------------------------
function [] = plot_sect_unflanged(ts, tw, bs, bw)
...
P3 = P2; P3(:,1) = P3(:,1) + bs;
% Skin
P4 = [-bs/2, 0; P3(end,1)+bs/2, 0];

scale = 1000;

figure, hold on
plot(P1(:,1).*scale, P1(:,2).*scale, 'ok-', 'LineWidth', 1, 'MarkerSize', 6, 'MarkerFaceColor','k')
plot(P2(:,1).*scale, P2(:,2).*scale, 'ok-', 'LineWidth', 1, 'MarkerSize', 6, 'MarkerFaceColor','k')
plot(P3(:,1).*scale, P3(:,2).*scale, 'ok-', 'LineWidth', 1, 'MarkerSize', 6, 'MarkerFaceColor','k')
plot(P4(:,1).*scale, P4(:,2).*scale, 'or-', 'LineWidth', 1, 'MarkerSize', 6, 'MarkerFaceColor','r')
xlabel('[mm]'), ylabel('[mm]')
title('GUESS - Unflanged, integrally stiffened shell geometry')

axis equal

end
%------------------------------------------------------------------------------------------------------------
function [] = plot_sect_stiffened(ts, tf, tw, bs, bf, bw)

% Zees
P1 = [0, 0; bf, 0; bf, bw; 2*bf, bw];
P2 = P1; P2(:,1) = P2(:,1) + bs;
P3 = P2; P3(:,1) = P3(:,1) + bs;
% Skin
P4 = [-bf/2, 0; P3(end,1)+bf/2, 0];
...
plot(P4(:,1).*scale, P4(:,2).*scale, 'or-', 'LineWidth', 1, 'MarkerSize', 6, 'MarkerFaceColor','r')
xlabel('[mm]'), ylabel('[mm]')
title('GUESS - Z-stiffened shell geometry')

axis equal

end
%------------------------------------------------------------------------------------------------------------
function [] = plot_sect_truss(tf, tc, bf, bc, h, theta)
...
plot(P3(:,1).*scale, P3(:,2).*scale, 'ok-', 'LineWidth', 1, 'MarkerSize', 6, 'MarkerFaceColor','k')
plot(P4(:,1).*scale, P4(:,2).*scale, 'or-', 'LineWidth', 1, 'MarkerSize', 6, 'MarkerFaceColor','r')
plot(P5(:,1).*scale, P5(:,2).*scale, 'or-', 'LineWidth', 1, 'MarkerSize', 6, 'MarkerFaceColor','r')
xlabel('[mm]'), ylabel('[mm]')
title('GUESS - Truss-core sandwich shell geometry')
