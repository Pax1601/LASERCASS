...
function [SOL, flag, fun, CUNS] = optim_Zst_sk_panel_9(C, H, n_strs, rib_pitch, E, smax,...
                                                       Nax, Tx, Ty, Mx, Mz, CEVAL, X0)
...
% OPTIONS =optimset('Algorithm','interior-point','MaxFunEvals',25000,'TolCon', 0.005, 'LargeScale', 'off', 'Display', 'off','tolfun',1e-4);
  ...
  warning off
  try
    [SOL, fun, flag, output] = fmincon(@(X)Zst_sk_mass_9(X,data), X0, ...
                               [],[],[],[], ... 
                               XL, XU, @(X)cstr_Zst_sk_panel_9(X,data), OPTIONS);
  %
    if (flag<0)
      if PRINT
        for (k=1:length(SOL))
      ...
      [CIN, CEQ] = cstr_Zst_sk_panel_9(SOL, data);
      if PRINT
        for (k=1:length(CIN))
