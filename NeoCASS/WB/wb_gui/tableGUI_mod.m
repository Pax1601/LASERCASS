...
function out = tableGUI_mod(varargin)
...
%
%
%   Modification (by Luca Riccobene)
%       24-Sep-2008 - Slight adjustment on figure properties: set to 'off'
%                     'IntegerHandle' property
%       27-Nov-2008 - Allowed slider option on Row Names too
%
...
hand.hFig = figure('unit','pixels','NumberTitle','off','Menubar','none','resize','on','position', ...
    pos,'Name',hand.FigName,'Resize','off','Visible','off', 'IntegerHandle', 'off');
...
if (~isempty(hand.RowNames))           % Create the header pushbutton uicontrols
    for (i = 1:length(hand.RowNames))
        hand.RNames(i) = uicontrol('Style','pushbutton','unit','pixels','Enable','inactive','position', ...
...
if (hand.NumRows > hand.MAX_ROWS)
    ...
    if (~isempty(hand.RNames))
        set(hand.RNames(hand.MAX_ROWS+1:hand.NumRows),'Visible','off')
    end
...
if (ds < 0)                                         % Slider moved down
    ...
    for (i = n:min(n+hand.MAX_ROWS-1,hand.NumRows))   % Update positions
        ...
        if (~isempty(hand.RNames))                  % If we have row names
            pos = hand.RowNames_pos_orig(i,:);
            set(hand.RNames(i),'pos',[pos(1) hand.arr_pos_y(i-d_col)+3 pos(3:4)],'Visible','on')            
        end
    end
...
elseif (ds > 0)                                     % Slider moved up
    ...
    for (i = n:-1:max(n-hand.MAX_ROWS+1,1))         % Update positions
        ...
        if (~isempty(hand.RNames))                  % If we have row names
            pos = hand.RowNames_pos_orig(i,:);
            set(hand.RNames(i),'pos',[pos(1) hand.arr_pos_y(k)+3 pos(3:4)],'Visible','on')            
        end
        k = k - 1;
    end
    set(hand.hEdits(n+1:end,1:end),'Visible','off')
    if (~isempty(hand.checks)),     set(hand.hChecks(n+1:end),'Visible','off');    end
    if (~isempty(hand.RNames)),     set(hand.RNames(n+1:end),'Visible','off');    end
