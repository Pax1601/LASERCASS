...
function [h, rho, p, T, mu] = ISA_a(vel)
...
% h
h = interp1(A(1:74,8), A(1:74,1), vel);
...
if h<0
  ...
  % density
  rho = interp1(A(1:74,8), A(1:74,5), vel);
  rho = rho .* rho_ssl;
  % pressure
  p = interp1(A(1:74,8), A(1:74,4), vel);
  p = p .* p_ssl;
  % Temperature
  T = interp1(A(1:74,8), A(1:74,3), vel);
  T = T + 273.15;
  % mu
  mu = interp1(A(1:74,8), A(1:74,6), vel);
  mu = mu .* mu_ssl;
