...
%***********************************************************************************************************************
% create interpolation matrix with SPLINE1
    ...
    IDPA=[AERO.lattice.DOF(n, 1, 1):AERO.lattice.DOF(n, 1, 2)];
      ...
      if (SET_AV(i)>0)
        ...
        for nl = 1: NP 
          ...
          ID  = [ID , (IDPA(PANEL(nl))-1)*6+1: IDPA(PANEL(nl))*6];
        end
        ID = ID';
        i1 = AERO.Interp.Param(ninterp, 1);
        i2 = AERO.Interp.Param(ninterp, 2);
        toll = AERO.Interp.Param(ninterp, 3);
        [Iv, Rmat] = beam_interface(str_data, aero_data, i1, i2, toll); % interface points in local frame
%------------------------------------------------------------------------------------------------------------------
...
%     Rt rotates aero displacements in the absolute frame
        ...
        for k=1:naer
          ri = [ri, ID(k)+tot_np*12];
          ci = [ci, k];
          vi = [vi, RmatT(3,3)];
        end
        nrows = double(18*tot_np);
        ncols = double(3*naer);
