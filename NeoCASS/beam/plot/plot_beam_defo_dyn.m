...
function H = plot_beam_defo_dyn(nfig, scale, varargin)
...
%
  ...
  Hpost = post_plot_beam_model(nfig, beam_model.Info, R, NODEPOS, beam_model.Bar, beam_model.PBar,...
...
%           fprintf(fid, '\nVLM lattice interpolated due to lack of deformed lattice.');
 	        lattice_defo = update_vlm_mesh(beam_model.Node, NODEPOS, R, beam_model.Aero);
        else
          lattice_defo = beam_model.Aero.lattice_defo;
        end
      end
	   Hvlm =  plot_vlm_model(lattice_defo, beam_model.Aero.ref, WAKE_PLOT, NORMAL_PLOT, ...
...
%      end
  ...
  if beam_model.Info.nconm
      ...
      Hconm =  plot_conm(beam_model.Info.nconm, NODEPOS, beam_model.ConM.Node, R, beam_model.ConM.Offset);
...
%*******************************************************************************
function H = post_plot_beam_model(nfig, INFO, NODER, COORD, BAR, PBAR, BEAM, PBEAM, BARR, BEAMR)
...
for n=1:ngrid
    Hgrid(n) = plot3(COORD(n, 1), COORD(n, 2), COORD(n, 3),'kx', 'MarkerSize', 4, 'MarkerFaceColor','k');
end
...
if nbar > 0
	...
	HBar = plot_3n_defo_line_elem(nbar, NODER, COORD, BAR, BARR, PBAR);

end

if nbeam > 0

	HBeam = plot_3n_defo_line_elem(nbeam, NODER, COORD, BEAM, BEAMR, PBEAM);
...
%*******************************************************************************
function H = plot_3n_defo_line_elem(nbar, NODER, COORD, BAR, BARR, PBAR)
...
for n=1:nbar
	...
	H((n-1)*14 +1) = plot3([COORD(BAR.Conn(n, 1),1) + offset1(1); COLLOC(1,1)], ...
	...
	H((n-1)*14 +2) = plot3([COORD(BAR.Conn(n, 2),1) + offset2(1); COLLOC(1,1)], ...
	  [COORD(BAR.Conn(n, 2),2) + offset2(2); COLLOC(1,2)], ...
	  [COORD(BAR.Conn(n, 2),3) + offset2(3); COLLOC(1,3)], ...
    '-r','LineWidth',1, 'MarkerSize', 3, 'MarkerFaceColor','r');
	
	H((n-1)*14 +3) = plot3([COORD(BAR.Conn(n, 2),1) + offset1(1); COLLOC(2,1)], ...
    [COORD(BAR.Conn(n, 2),2) + offset1(2); COLLOC(2,2)], ...
    [COORD(BAR.Conn(n, 2),3) + offset1(3); COLLOC(2,3)],...
     '-r','LineWidth',1, 'MarkerSize', 3, 'MarkerFaceColor','r');

	H((n-1)*14 +4) = plot3([COORD(BAR.Conn(n, 3),1) + offset2(1); COLLOC(2,1)], ...
	...
	H((n-1)*14 +5) = plot3([COLLOC(:, 1)]', [COLLOC(:, 2)]', [COLLOC(:, 3)]', 'rs', 'MarkerSize', ...
    2, 'MarkerFaceColor','r');

	H((n-1)*14 +6) = plot3([N1(1) N2(1) N3(1)]', [N1(2) N2(2) N3(2)]', [N1(3) N2(3) N3(3)]', 'ro', ...
  ...
  for k=1:4
    ...
    X1(2,:) = COLLOC(1, :);
	H((n-1)*14 + 6 + (k-1)*4+1) = 	plot3(X1(:,1),X1(:,2),X1(:,3),...
...
%
  for k=1:4
    ...
    X1(2,:) = COLLOC(2, :);
	H((n-1)*14 +(k-1)*4+1 +10) = 	plot3(X1(:,1),X1(:,2),X1(:,3),...
    '--yv','LineWidth',1, 'MarkerSize', 2, 'MarkerFaceColor','y');
  end
...
%*******************************************************************************
function H = plot_vlm_model(lattice, ref, WAKE_PLOT, NORMAL_PLOT, C_mac)

	H1 = plot3(lattice.XYZ(:,:,1)',lattice.XYZ(:,:,2)',lattice.XYZ(:,:,3)','-bo',...
    'MarkerSize', 1, 'MarkerFaceColor','b');
	
	H2 = plot3(lattice.COLLOC(:,1),lattice.COLLOC(:,2),lattice.COLLOC(:,3),'bx',...
	...
	H3 = plot3(v1, v2, v3,'bo',  'MarkerSize', 1, 'MarkerFaceColor','b');
...
%*******************************************************************************
function H = plot_conm(nconm, Coord, conm_node, R, offset)
H = zeros(nconm*2,1);
  for n=1:nconm
	...
	H((n-1)*2 + 1) = plot3(cx, cy, cz, '--r', 'MarkerSize', 4, ...
    'MarkerFaceColor','r');
	H((n-1)*2 + 2) = plot3(cx(1), cy(1), cz(1), '--ro', 'MarkerSize', 4, ...
    'MarkerFaceColor','r');
