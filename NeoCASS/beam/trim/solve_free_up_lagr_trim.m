...
prestresses = [];
previous_alpha = 0;
delta_matrix = zeros(num_repeat, num_repeat);
%
if nargin
    PARAM = varargin;
...
for counter = 1: num_repeat
    if (~isempty(find(beam_model.Param.MSOL == 144)))
        ...
        for ntrim = 1:2
            ...
            UDD(:,ntrim) = 1 / num_repeat * beam_model.Aero.Trim.FM.Value(7:12)';
            %
            if ~isempty(indexa)
...
%             beam_model.Aero.lattice_vlm.Control.Hinge,beam_model.Aero.ID);
        ...
        previous_alpha = previous_alpha + UX(1, 2);
        
        delta_i = UX(6, 2);
        
        if (counter == 1)
            delta_matrix(1, 1) = delta_i;
        else
            delta_matrix(counter, :) = delta_matrix(counter - 1, :);
            delta_matrix(counter, counter) = delta_i;
        end
        
        
        previous_alpha * 180 / pi
...
delta_steps = delta_matrix * ones(num_repeat, 1);
delta_tot = sum(delta_steps) * 180 / pi
