...
%***********************************************************************************************************************
function solve_free_up_lagr_trim(varargin)
...
old_fid = beam_model.Param.FID;
beam_model.Param.FID = fopen('output.txt', 'wt');
...
% make a copy of the beam_model structure to reset it after the iterations
temp_model = beam_model;
% number of loads steps for the Updated Lagrangian method
NSTEP = beam_model.Param.NSTEP;
NITER = beam_model.Param.NITER;
% vector of prestresses to compute the prestress stiffness matrix
prestresses = zeros(2, 6, beam_model.Info.nbar);
% trim variables vector updated at each step
tot_UX = [];
% total residual acceleration tolerance 
...
trim_FN = zeros(length(beam_model.Aero.lattice_vlm.N), 1);
...
if ~isempty(index)
    ...
    fprintf(fid,'\nSolving static unrestrained trim with Updated Lagrangian method (ID %d)...\n\n', beam_model.Aero.Trim.ID(index));
    %
...
beam_model.Aero.Trim.CS.MPC = [];   % store control surfaces constraint equations
beam_model.Aero.Trim.CS.Coeff = []; % store control surfaces constraint coefficients

% set output struct

beam_model.Res = [];
beam_model.Res.SOL = 'Static unrestrained trim with Updated Lagrangian method';
...
% Updated Lagrangian iteration. The last iteration is used to computed the
% final accelerations acting on the structure, hence the total steps are NSTEP + 1
for step = 1: NSTEP + 1
    
    if step <= NSTEP
        fprintf('\nStep %i of %i', step, NSTEP)
    end
    
    if step == 1
        tot_UX = zeros(length(find(beam_model.Aero.Trim.CS.MPC == 0))+5, 1);
    end
    ...
    % Inner loop. The while loop is stopped when the acceleration residual
    % is smaller than the provided tolerance.
    iteration = 0;
    for iter = 1: NITER
        ...
        if (~isempty(find(beam_model.Param.MSOL == 144)))
            ...
            if FLAG ~= 1
                ...
                % using the prestresses to compute the geometric stiffness
                % matrix
                ...
                % External forces are applied as increments
                F_ext = step / NSTEP * gf_lin_nodal(LOADI, beam_model.Info, beam_model.F, beam_model.M, beam_model.Node.DOF);
                F_flw = step / NSTEP *  gf_flw_nodal(beam_model.Param.LOAD, beam_model.Info, beam_model.F_FLW, beam_model.Node.R, ...
            ...
            % To linearize the system around the position obtained at the
            % previous step, a set of reference values is used, obtained from
            % the updated trim vector tot_UX

            % Values around which the aerodynamics are linearized.
            ref_alpha = tot_UX(1);
            ref_betha = tot_UX(2);
            ref_P = tot_UX(3);
            ref_Q = tot_UX(4);
            ...
            % The airplane center of gravity is updated to compute the
            ...
            dummy_aero.state.alpha = ref_alpha; dummy_aero.state.betha = ref_betha;
            dummy_aero.state.P = ref_P;     dummy_aero.state.Q = ref_Q;      dummy_aero.state.R = ref_R;
            %-------------------------------------------------------------------------------
            ...
            % At the final iteration, the process is stopped and the loop is
            % exited
            if (step == NSTEP + 1)
                break;
            end
            
            % Resultant forces and moments acting on the airplane,
            % evaluated at the CG
            F_vect = [results0.FORCES results0.MOMENTS(2, :)]';
            ...
            req_acc = zeros(ngrid, 1);
            req_acc(rdof) = (step) / NSTEP * beam_model.Aero.Trim.FM.Value(7:12)';
            req_acc(ldof) = D * req_acc(rdof);    
            
            Fin = M * req_acc;
            
            F = -Fel - Fin + Fa0 + F_ext + F_flw;
            

            % The accelerations are computed from the mass matrix evaluated
            % at the center of gravity
            acc_residual = beam_model.WB.MCG \ F_vect - (step) / NSTEP * beam_model.Aero.Trim.FM.Value(7:12)';
            
            fprintf(' residual error: %f.', norm(F));
            ...
            dummy_aero = beam_model.Aero;
            dummy_aero.state.alpha = ref_alpha + EPS; dummy_aero.state.betha = ref_betha;
            dummy_aero.state.P = ref_P;     dummy_aero.state.Q = ref_Q;      dummy_aero.state.R = ref_R;
            ...
            dummy_aero.state.alpha = ref_alpha; dummy_aero.state.betha = ref_betha + EPS;
            dummy_aero.state.P = ref_P;     dummy_aero.state.Q = ref_Q;      dummy_aero.state.R = ref_R;
            ...
            dummy_aero.state.alpha = ref_alpha; dummy_aero.state.betha = ref_betha;
            dummy_aero.state.P = ref_P + EPS;     dummy_aero.state.Q = ref_Q;      dummy_aero.state.R = ref_R;
            ...
            dummy_aero.state.alpha = ref_alpha; dummy_aero.state.betha = ref_betha;
            dummy_aero.state.P = ref_P;     dummy_aero.state.Q = ref_Q + EPS;      dummy_aero.state.R = ref_R;
            ...
            dummy_aero.state.alpha = ref_alpha; dummy_aero.state.betha = ref_betha;
            dummy_aero.state.P = ref_P;     dummy_aero.state.Q = ref_Q;      dummy_aero.state.R = ref_R + EPS;
            ...
            for ntrim = 1:2
                ...
                % The required accelerations are divided by the numbers of
                % steps
                UDD(:,ntrim) = zeros(size(beam_model.Aero.Trim.FM.Value(7:12)'));
                ...
                if ntrim==1
                    ...
                    beam_model.Res.Aero.RTrim_sol = store_trim_sol(fid, UDD(:,1), UX(:,1), beam_model.Res.Aero.RStab_Der.Control.Name, 1);
                    fprintf(outp, '\n\nRIGID TRIM RESULTS\n');
                    dummy = store_trim_sol(outp, UDD(:,1), UX(:,1), beam_model.Res.Aero.RStab_Der.Control.Name, 1);
                    ...
                    beam_model.Res.Aero.DTrim_sol = store_trim_sol(fid, UDD(:,2), UX(:,2), beam_model.Res.Aero.DStab_Der.Control.Name, 1);
                    fprintf(outp, '\n\nELASTIC TRIM RESULTS\n');
                    dummy = store_trim_sol(outp, UDD(:,2), UX(:,2), beam_model.Res.Aero.DStab_Der.Control.Name, 1);
            ...
            % update the mass matrix and CG position
            beam_model.WB.MRP = beam_model.Res.WB.MRP;
            beam_model.WB.MCG = beam_model.Res.WB.MCG;
            beam_model.WB.CG =  beam_model.Res.WB.CG;
            ...
            %         for r = 1: length(beam_model.Res.Aero.F0_DTrim)
            %             trim_FN(r) = trim_FN(r) + beam_model.Res.Aero.F0_DTrim(r, :) * beam_model.Aero.lattice_vlm.N(r, :)';
            %         end
            ...
            % Update the aerodynamic mesh for the next step
            beam_model.Aero.lattice_vlm = beam_model.Aero.lattice_defo;
            
            % Update the structural grid points position and rotation
            beam_model.Node.Coord = beam_model.Node.Coord + beam_model.Res.NDispl(:,1:3);
            for repeat = 1: size(beam_model.Res.NRd, 3)
                beam_model.Node.R(:, :, repeat) = beam_model.Node.R(:, :, repeat) * beam_model.Res.NRd(:, :, repeat);
            end
            
            % Update the prestress vector to compute the geometric stiffness
            % matrix
            
            for l = 1: beam_model.Info.nbar
           
                rmat = beam_model.Bar.R(:, :, 4, l);
                new_force = beam_model.Res.Bar.CForces(1, 1:3, l) * rmat';
                new_moment = beam_model.Res.Bar.CForces(1, 4:6, l) * rmat';
                prestresses(1, :, l) = prestresses(1, :, l) + [new_force new_moment];

                rmat = beam_model.Bar.R(:, :, 5, l);
                new_force = beam_model.Res.Bar.CForces(2, 1:3, l) * rmat';
                new_moment = beam_model.Res.Bar.CForces(2, 4:6, l) * rmat';
                prestresses(2, :, l) = prestresses(2, :, l) + [new_force new_moment];

            end
            
            
            % Update the total trim parameters
            tot_UX = tot_UX + UX(:, 2);
...
% The accelerations are computed from the mass matrix evaluated
% at the center of gravity
req_acc = beam_model.WB.MCG \ F_vect;
...
store_trim_sol(fid, req_acc, tot_UX, beam_model.Res.Aero.DStab_Der.Control.Name, 1);
fprintf('\n');

% Restore the beam_model structure and compute the node deformation for
% plotting
beam_model.Aero.lattice_defo = beam_model.Aero.lattice_vlm;
beam_model.Aero.lattice_vlm = temp_model.Aero.lattice_vlm;
...
fclose(beam_model.Param.FID);
beam_model.Param.FID = old_fid;
