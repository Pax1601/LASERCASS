...
%
function solve_free_lin_land(TRIM_INDEX, outp)
...
%
    if ntrim==1
      ...
      beam_model.Res.Aero.DTrim_sol = UDD(:,2);
      print_trim_sol(fid, UDD(:,2));
      fprintf(outp, '\n\nCORRECTIONS TO ELASTIC TRIM\n');
      print_trim_sol(outp, UDD(:,2));
    end
  fprintf(fid,'\ndone.');
  end % trim loop%
...
%
  UR = -M4RR * UDD(:,ntrim)  - TMP2; 
  UL = -AMLR * UDD(:,ntrim) - ARLR * UR  + UINTL;
%
