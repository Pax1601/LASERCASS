...
function [N, Kp] = set_Kp_mat(f1, f2, f3, PRESTRESS)
...
tI0 = PRESTRESS(1, 1:2:5);
tII0 = PRESTRESS(2, 1:2:5);

Kp(4:6,1:3) = crossm(tI0 .* NI(1)); %AGGIUNGERE TERMINE CON DELTA
...
Kp(10:12,1:3) = -crossm(tI0 .* NI(1)) + crossm(tII0 .* NII(1)); %AGGIUNGERE TERMINE CON DELTA
Kp(10:12,4:6) = crossm(tI0 .* NI(1)) * crossm(diag([f1, f2, f3])) - crossm(tII0 .* NII(1)) * crossm(diag([f1, f2, f3]));

Kp(10:12,7:9) = -crossm(tI0 .* NI(2)) + crossm(tII0 .* NII(2)); %AGGIUNGERE TERMINE CON DELTA
Kp(10:12,10:12) = crossm(tI0 .* NI(2)) * crossm(diag([f1, f2, f3])) - crossm(tII0 .* NII(2)) * crossm(diag([f1, f2, f3]));

Kp(10:12,13:15) = -crossm(tI0 .* NI(3)) + crossm(tII0 .* NII(3)); %AGGIUNGERE TERMINE CON DELTA
Kp(10:12,16:18) = crossm(tI0 .* NI(3)) * crossm(diag([f1, f2, f3])) - crossm(tII0 .* NII(3)) * crossm(diag([f1, f2, f3]));


Kp(16:18,1:3) = crossm(tII0 .* NII(1)); %AGGIUNGERE TERMINE CON DELTA
...
Kp(16:18,13:15) = crossm(tII0 .* NII(3));
