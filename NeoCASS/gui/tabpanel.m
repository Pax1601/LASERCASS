...
%
%   Modified by Luca Riccobene (luca.riccobene@polimi.it) to support Matlab
%   versions higher than R2014a
%
...
if nargin == 3 && ischar(action)
   ...
   %%%%% Modified here - begin
   % LR: starting from Matlab R2014b - version 8.4.0.150421 - the graphical
   % objects change substantially
   if verLessThan('matlab', '8.4')
       tab = get(handles.(tag)(end),'Userdata');
   else
       tmptab = handles.figure1.Children.findobj('tag', 'Main');
       tab    = tmptab(end).UserData;
   end
   %%%%% Modified here - end
   ...
   switch action
      case 'edit'
         ...
         %%%%% Modified here - begin
         % LR: added try ... end since 'constructor_(tag)' can't be found
         try
      ...
      case 'update'
         ...
         for j={figfiles.name}
            ...
            % LR fixed for compatibility with R2014b
%             copyobj(tmp(end:-1:1),fig);
            copyobj(tmp(end:-1:1),fig, 'legacy');
            close(hpage);
            delete(char(j));
         end% for
         try
...
if ~isempty(get(handles.(tag),'Userdata'))
   ...
   modStrCallback    = ['if verLessThan(''matlab'', ''8.4'')'...
'tpctab = get(tpchandles.(get(gcbo,''Tag''))(end),''UserData'');'...
'else'...
'tmptab = handles.figure1.Children.findobj(''tag'', ''Main''); ' 'tpctab = tmptab(end).UserData;'...
'end'];
