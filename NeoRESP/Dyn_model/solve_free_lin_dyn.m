...
%
%
function solve_free_lin_dyn(varargin)
...
%
% sampling frequencies for half time story          
%
  Omega = 2*pi * ([0:nt-1]')./((2*nt-1) * Dt);
...
%
  ...
  fprintf(fid,'\n - Frequency step         : %g Hz.', 1/((2*nt-1) * Dt));
...
%    dyn_model.dlm.data.Qdh(:,UMODESRGLOB(side_mode),1,MINDEX) = 0;
%    dyn_model.dlm.data.Qdh(:,UMODESRGLOB(plunge_mode),1,MINDEX) = 0;
%    dyn_model.dlm.data.Qdh(:,UMODESRGLOB(roll_mode),1,MINDEX) = 0;
...
% control input
  if  ~isempty(beam_model.Param.SURFDEF)
    ...
    for i = 1: nSURF
      ftime = zeros(nt,1);
      for n=1:beam_model.Surfdef.Npiece
        DELAY = abs(beam_model.Surfdef.Ti(beam_model.Param.SURFDEF(i)).data(n,1));
        ind = find(T >= DELAY & T<= DELAY + beam_model.Surfdef.Ti(beam_model.Param.SURFDEF(i)).data(n,2));
        t = T(ind) - DELAY;
        fun = Scalar2VectorialFun(beam_model.Surfdef.fun{beam_model.Param.SURFDEF(i),n});
...
% Gust
  if ~isempty(beam_model.Param.GUST)
    ...
    for i = 1:nGUST
      ...
      for j=1:nk
        Hag(:,i,j) = dyn_model.gust.Qhg(:,:,j,MINDEX) * Qhg(:,j);
      end
      dyn_model.dlm.data.Hag = Hag;
%
...
%    Kmg(:,pitch_mode) = Kmg(:,pitch_mode) + beam_model.Struct.V(:, UMODESIND)' * beam_model.Struct.M * grav_load;  
%  end
%
%  roll_mode = find(UMODES == 4);
...
%    Kmg(:,roll_mode) = Kmg(:,roll_mode) + beam_model.Struct.V(:, UMODESIND)' * beam_model.Struct.M * grav_load;  
%  end
%
...
%   system dynamics
    Aow = MASS + Bmm(UMODESIND,UMODESIND)* 1i * Omega(i) + Kmm(UMODESIND,UMODESIND) + Kmg - qinfty * HAM;
%   response
    Q(:,i) = Aow \ L_tot(:,i);
...
% save dynamics
  ...
  dyn_model.Res.Qload = ltot_mode(:,1:nt);
  dyn_model.Res.Qextf = ltot_extf(:,1:nt);
% clean rigid modes
  q(UMODESRGLOB,:) = bsxfun(@minus,q(UMODESRGLOB,:),q(UMODESRGLOB,1));
