function [r,J] = MyIdentification_Adjugate_Fun_4LM(thetaold,ord,p,H,H0,weight,ro)
% =========================================================================
%                                         MyIdentification_Adjugate_Fun_4LM
% =========================================================================
%
% Description: Adjugate-based method
%
% -------------------------------------------------------------------------
%
...
orderD = ord;
orderN = ord + ro;
[D,N] = extractmatrices( 'left', 'adj', thetaold, H0, orderD, orderN );
...
for k = kinit:nk
    ...
    % calculate determinant and impose overflow limit
    detD = det(Dk);    
    detDinv = 1/detD;
    Mthreshold = 10^9;
    if abs(detDinv) > Mthreshold, detDinv = weight; end
    
    % Jacobian
    Jac = zeros( du, dy*(ord) + du*(ord+ro) );
    Jac(:, 1:dy) =  H0'-Happrox';
    for m = 1 : (ord-1)
