...
function [Hsa, knew] = iner2sa(Ham, k, cref, ORDER)
%
  kstep = 0.05;
  kint = [0:kstep:k(end)];
  fid = 1;
...
%
  elseif nc==6
    ...
    R0(5,5) = 1;
    for i=1:nk
      Ham(1:6,:,i) = R0 * Ham(1:6,:,i);
    end
  else
    error('Wrong number of columns for Ham matrix. Five or six columns allowed.');
  end
%
